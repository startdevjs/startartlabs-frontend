...
const FriendCard = ({
  ...
  const handleFriendshipExists = (userId) => {
    const isFriendshipAlreadyExists = allMyFriends?.data?.friends.find(
  ...
  const handleFriendshipId = (userId) => {
    const isFriendshipAlreadyExists = sentFriendshipRequests.find(
      (user) => user.friendId === userId,
    );
    setRevalidate((prev) => !prev);
    if (isFriendshipAlreadyExists) {
        ...
        <Card>
          ...
          >
            ...
            <div className="tile__container">
              <p className="tile__title m-0" style={{ fontSize: "12px" }}>
                {user?.name}
              </p>
            </div>
          </div>
          {handleFriendshipExists(user?.id) ? (
            ...
            <Button
              ...
              style={{ backgroundColor: "${({ theme: { colors } }) => colors.tertiaryColor}" }}
              disabled={false}
              onClick={() => {
